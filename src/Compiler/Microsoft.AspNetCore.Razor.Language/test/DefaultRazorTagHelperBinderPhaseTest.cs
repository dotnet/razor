...
@addTagHelper """;
        ...
        var erroredNode = rewrittenTree.Root.DescendantNodesAndTokens().First(n => n.GetChunkGenerator() is AddTagHelperChunkGenerator);
...
@removeTagHelper """;
        ...
        // Assert
        var rewrittenTree = codeDocument.GetSyntaxTree();
        var erroredNode = rewrittenTree.Root.DescendantNodesAndTokens().First(n => n.GetChunkGenerator() is RemoveTagHelperChunkGenerator);
...
@tagHelperPrefix """;
        ...
        // Assert
        var rewrittenTree = codeDocument.GetSyntaxTree();
        var erroredNode = rewrittenTree.Root.DescendantNodesAndTokens().First(n => n.GetChunkGenerator() is TagHelperPrefixDirectiveChunkGenerator);
    ...
    public void Execute_RewritesTagHelpers()
    {
        ...
        // Assert
        var rewrittenTree = codeDocument.GetSyntaxTree();
        var descendantNodes = rewrittenTree.Root.DescendantNodesAndTokens();
...
";
        ...
        var result = Assert.Single(results);
        Assert.Same(componentDescriptor, result);
        var directiveChunkGenerator = (TagHelperPrefixDirectiveChunkGenerator)tree.Root.DescendantNodesAndTokens().First(n => n is CSharpStatementLiteralSyntax).GetChunkGenerator();
