...
internal class ComponentPageDirectivePass : IntermediateNodePassBase, IRazorDirectiveClassifierPass
{
    protected override void ExecuteCore(RazorCodeDocument codeDocument, DocumentIntermediateNode documentNode)
    {
        ...
        foreach (var (_, pageDirective) in directives)
        {
            if (codeDocument.FileKind.IsComponentImport() || pageDirective.IsImported)
            {
                pageDirective.AddDiagnostic(ComponentDiagnosticFactory.CreatePageDirective_CannotBeImported(pageDirective.Source.GetValueOrDefault()));
            }
        }
