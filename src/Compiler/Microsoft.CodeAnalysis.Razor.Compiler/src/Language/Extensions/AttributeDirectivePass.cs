...
internal class AttributeDirectivePass : IntermediateNodePassBase, IRazorDirectiveClassifierPass
{
    protected override void ExecuteCore(RazorCodeDocument codeDocument, DocumentIntermediateNode documentNode)
    {
        ...
        foreach (var (_, attribute) in documentNode.FindDirectiveReferences(AttributeDirective.Directive))
        {
            var token = attribute.Tokens.FirstOrDefault();
            if (token != null)
            {
                ...
                node.Children.Add(NodeFactory.CSharpToken(token.Content, token.Source));
