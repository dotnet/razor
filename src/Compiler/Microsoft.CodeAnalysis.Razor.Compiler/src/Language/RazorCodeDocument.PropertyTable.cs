...
{
    /// <summary>
    ///  Represents an immutable set of values associated with a <see cref="RazorCodeDocument"/>.
    /// </summary>
    private readonly struct PropertyTable
    {
        // To add a value, increase Size by 1 and add a new property below.
        ...
        private PropertyTable(object?[] values)
        {
            _values = values;
        }
        ...
        public PropertyTable With<T>(Property<T> property, T? value) where T : class
        {
            var newValues = new object?[Size];
            Array.Copy(_values, newValues, Size);
            newValues[property.Index] = value;
            return new PropertyTable(newValues);
        }

        public PropertyTable With<T>(BoxedProperty<T> property, T value) where T : struct
        {
            var newValues = new object?[Size];
            Array.Copy(_values, newValues, Size);
            newValues[property.Index] = new StrongBox<T>(value);
            return new PropertyTable(newValues);
        }
        ...
        /// <summary>
        ///  Provides read-only access to a specific slot within an array for a given reference type.
        /// </summary>
        ...
        /// <summary>
        ///  Provides read-only access to a specific slot within an array for a given value type.
