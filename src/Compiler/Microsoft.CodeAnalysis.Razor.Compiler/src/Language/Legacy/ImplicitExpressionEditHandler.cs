...
{
    public static void SetupBuilder(SpanEditHandlerBuilder builder, Func<string, IEnumerable<Syntax.InternalSyntax.SyntaxToken>> tokenizer, bool acceptTrailingDot, ImmutableHashSet<string> keywords)
    {
    ...
    private bool IsAcceptableIdentifierReplacement(SyntaxNode target, SourceChange change)
    {
        ...
        var tokens = target.DescendantNodesAndTokens().Where(n => n.IsToken).Cast<SyntaxToken>().ToArray();
