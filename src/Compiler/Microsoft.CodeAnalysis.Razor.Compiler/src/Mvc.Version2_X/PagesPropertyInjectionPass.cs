...
public class PagesPropertyInjectionPass : IntermediateNodePassBase, IRazorOptimizationPass
{
    protected override void ExecuteCore(RazorCodeDocument codeDocument, DocumentIntermediateNode documentNode)
    {
        ...
        vddProperty.Children.Add(NodeFactory.CSharpToken($"public {viewDataType} ViewData => ({viewDataType})PageContext?.ViewData;"));

        @class.Children.Add(vddProperty);

        var modelProperty = new CSharpCodeIntermediateNode();
        modelProperty.Children.Add(NodeFactory.CSharpToken($"public {modelType} Model => ViewData.Model;"));
