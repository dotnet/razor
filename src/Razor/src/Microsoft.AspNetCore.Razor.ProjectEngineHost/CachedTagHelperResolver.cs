...
internal partial class CachedTagHelperResolver(ITelemetryReporter telemetryReporter)
{
    ...
    public ImmutableArray<TagHelperDescriptor> GetValues(ProjectId projectKey, int resultId)
    {
    ...
    protected TagHelperDescriptor? TryGet(Checksum checksum)
    {
        TagHelperCache.Default.TryGet(checksum, out var tagHelper);
        return tagHelper;
    }

    protected async ValueTask<ImmutableArray<Checksum>> GetCurrentChecksumsAsync(Project project, CancellationToken cancellationToken)
    {
