        ...
        IRequestHandler<DocumentDiagnosticsParams, DiagnosticReport[]?>
    {
        ...
        internal async override Task<DiagnosticReport[]?> HandleRequestAsync(DocumentDiagnosticsParams request, ClientCapabilities clientCapabilities, string token, CancellationToken cancellationToken)
        {
            ...
            if (!_lspProgressListener.TryListenForProgress(
                token,
                onProgressNotifyAsync: (value, ct) => ProcessDocumentDiagnosticsAsync(value, request.PartialResultToken, ct),
            ...
            var remappedResults = await RemapDocumentDiagnosticsAsync(result, cancellationToken).ConfigureAwait(false);
        ...
        private async Task<DiagnosticReport[]?> RemapDocumentDiagnosticsAsync(DiagnosticReport[]? diagnosticReport, CancellationToken cancellationToken)
        {
            var remappedLocations = new List<DiagnosticReport>();
