...
{
    internal class VSLSPTagHelperTooltipFactory : TagHelperTooltipFactoryBase<VSClassifiedTextElement>
    {
        ...
        private static void ClassifyReducedTypeName(List<VSClassifiedTextRun> runs, string reducedTypeName)
        {
            ...
            for (var i = 0; i < reducedTypeName.Length; i++)
            {
                ...
                if (ch == '<' || ch == '>' || ch == '[' || ch == ']' || ch == ',')
                {
                    ...
                    {
                        var currentRunTextStr = currentRunText.ToString();
            ...
            if (currentRunText.Length != 0)
            {
                ClassifyShortName(runs, currentRunText.ToString());
            }
        }
        ...
        internal static void CleanAndClassifySummaryContent(List<VSClassifiedTextRun> runs, string summaryContent)
        {
            ...
            var successfulCrefMatches = new List<Match>();
            foreach (Match match in crefMatches)
            {
                if (match.Success)
                {
                    successfulCrefMatches.Add(match);
                }
            }

            if (successfulCrefMatches.Count == 0)
            {
            ...
            {
                // If we made it through all the crefs matches, add the rest of the text and break out of the loop.
                if (currentMatchIndex == successfulCrefMatches.Count)
                {
                    currentTextRun.Append(summaryContent.Substring(i));
                    break;
                }

                var currentMatch = successfulCrefMatches[currentMatchIndex];
                if (i == currentMatch.Index)
                {
                    // We reached a cref. The first thing we want to do before processing the cref is process
                    // the existing string we've been building up.
                    ...
                    var value = currentMatch.Groups[2].Value;
