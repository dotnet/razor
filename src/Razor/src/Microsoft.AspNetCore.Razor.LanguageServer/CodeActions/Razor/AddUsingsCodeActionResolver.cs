...
internal sealed class AddUsingsCodeActionResolver : IRazorCodeActionResolver
{
        ...
        List<RazorUsingDirective> existingUsingDirectives)
    {
        ...
        foreach (var usingDirective in existingUsingDirectives)
        {
            ...
            {
                var usingDirectiveLineIndex = codeDocument.Source.SourceText.Lines.GetLinePosition(usingDirective.Node.Span.Start).Line;
    ...
    {
        if (endIndex < codeDocument.Source.SourceText.Length)
        {
            return codeDocument.Source.SourceText.Lines.GetLinePosition(endIndex).Line;
        }
        else
        {
            return codeDocument.Source.SourceText.Lines.Count;
        }
    }
