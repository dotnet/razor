...
internal sealed class RemoteCompletionService(in ServiceArgs args) : RazorDocumentServiceBase(in args), IRemoteCompletionService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (provisionalTextEdit is not null)
        {
            ...
            generatedDocument = (SourceGeneratedDocument)ExternalAccess.Razor.Cohost.Handlers.Completion.ApplyProvisionalEdit(generatedDocument, generatedText.WithChanges([change]));
        }
