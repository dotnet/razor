...
internal static partial class RazorCodeDocumentExtensions
{
    ...
    /// <summary>
    /// Checks if the component class name (from @classname directive or file name) matches the given name.
    /// </summary>
    public static bool ComponentClassNameMatches(this RazorCodeDocument razorCodeDocument, ReadOnlyMemory<char> className)
    {
        var documentNode = razorCodeDocument.GetRequiredDocumentNode();
        var componentClassName = documentNode.FindPrimaryClass()?.Name;
        if (componentClassName is null)
        {
            return false;
        }

        return componentClassName.Content.Equals(className.Span, StringComparison.Ordinal);
    }
