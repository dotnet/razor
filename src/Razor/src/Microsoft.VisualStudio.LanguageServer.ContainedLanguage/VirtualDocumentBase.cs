...
public abstract class VirtualDocumentBase<T> : VirtualDocument where T : VirtualDocumentSnapshot
{
    ...
    public override VirtualDocumentSnapshot Update(IReadOnlyList<ITextChange> changes, int hostDocumentVersion, object? state)
    {
        ...
        if (TextBuffer.ContentType.DisplayName.Contains("CSharp"))
        {
        ...
        if (TextBuffer.ContentType.DisplayName.Contains("CSharp"))
        {
            LSPDocumentSynchronizer.LogFunction($"Finished calling update for {this.Uri} for version {_hostDocumentSyncVersion}, which produced snapshot version {snapshot.Version.VersionNumber} with {snapshot.LineCount} lines");
        }
