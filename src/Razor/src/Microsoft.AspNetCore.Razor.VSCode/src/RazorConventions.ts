...
export function isRazorCSharpFile(uri: vscode.Uri): boolean {
    const path = getUriPath(uri);
    return path.endsWith(virtualCSharpSuffix);
}

export function isRazorHtmlFile(uri: vscode.Uri): boolean {
    const path = getUriPath(uri);
    return path.endsWith(virtualHtmlSuffix);
}

export function getRazorDocumentUri(uri: vscode.Uri): vscode.Uri {
    const path = getUriPath(uri);
