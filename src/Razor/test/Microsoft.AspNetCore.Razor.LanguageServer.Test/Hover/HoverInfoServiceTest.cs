...
public class HoverInfoServiceTest : TagHelperServiceTestBase
{
    ...
    public async Task Handle_Hover_SingleServer_CallsDelegatedLanguageServer()
    {
        ...
        languageServerMock
            .Setup(c => c.SendRequestAsync<IDelegatedParams, VSInternalHover>(RazorCustomLSPMethodNames.RazorHoverEndpointName, It.IsAny<DelegatedPositionParams>(), It.IsAny<CancellationToken>()))
    ...
    private class HoverLanguageServer : ClientNotifierServiceBase
    {
        ...
        {
            Assert.Equal(RazorCustomLSPMethodNames.RazorHoverEndpointName, method);
