...
namespace Microsoft.AspNetCore.Razor.LanguageServer.CodeActions
{
    public class ExtractToCodeBehindCodeActionResolverTest : LanguageServerTestBase
    {
        ...
        {
            // Arrange
            var resolver = new ExtractToCodeBehindCodeActionResolver(new DefaultSingleThreadedDispatcher(), EmptyDocumentResolver, FilePathNormalizer);
        ...
        public async Task Handle_Unsupported()
        {
            ...
            var resolver = new ExtractToCodeBehindCodeActionResolver(new DefaultSingleThreadedDispatcher(), CreateDocumentResolver(documentPath, codeDocument), FilePathNormalizer);
            var data = JObject.FromObject(new ExtractToCodeBehindCodeActionParams()
            {
                Uri = new Uri("c:/Test.razor"),
                RemoveStart = 14,
