...
public class DocumentOnTypeFormattingEndpointTest : FormattingLanguageServerTestBase
{
    ...
    public async Task Handle_OnTypeFormatting_FormattingDisabled_ReturnsNull()
    {
        ...
        var documentMappingService = new RazorDocumentMappingService(TestDocumentFilePathProvider.Instance, new TestDocumentContextFactory(), LoggerFactory);
...
@{
 if(true){}
        ...
        var formattingService = new DummyRazorFormattingService();
        var documentMappingService = new RazorDocumentMappingService(TestDocumentFilePathProvider.Instance, documentResolver, LoggerFactory);
