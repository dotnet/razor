...
public class RenameEndpointTest(ITestOutputHelper testOutput) : LanguageServerTestBase(testOutput)
{
    ...
    public async Task Handle_Rename_SingleServer_CallsDelegatedLanguageServer()
    {
        ...
        documentMappingServiceMock
            .Setup(x => x.TryMapToGeneratedDocumentPosition(It.IsAny<RazorCSharpDocument>(), It.IsAny<int>(), out projectedPosition, out projectedIndex))
            .Returns(true);
        ...
        IClientConnection? clientConnection = null)
    {
        ...
        if (documentMappingService == null)
        {
            ...
            var projectedIndex = 1;
            documentMappingServiceMock
                .Setup(c => c.TryMapToGeneratedDocumentPosition(It.IsAny<RazorCSharpDocument>(), It.IsAny<int>(), out projectedPosition, out projectedIndex))
                .Returns(true);
