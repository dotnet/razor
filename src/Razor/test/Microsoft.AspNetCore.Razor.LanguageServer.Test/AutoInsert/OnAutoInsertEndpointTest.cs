...
namespace Microsoft.AspNetCore.Razor.LanguageServer.Formatting
{
    public class OnAutoInsertEndpointTest : LanguageServerTestBase
    {
        ...
        public async Task Handle_SingleProvider_InvokesProvider()
        {
            ...
            var endpoint = new OnAutoInsertEndpoint(Dispatcher, documentResolver, new[] { insertProvider }, TestLanguageServerWorkspaceFactory.Instance);
        ...
        public async Task Handle_MultipleProviderSameTrigger_UsesSuccessful()
        {
            ...
            var endpoint = new OnAutoInsertEndpoint(Dispatcher, documentResolver, new[] { insertProvider1, insertProvider2 }, TestLanguageServerWorkspaceFactory.Instance);
            var @params = new OnAutoInsertParams()
            {
                TextDocument = new TextDocumentIdentifier(uri),
                Character = ">",
        ...
        public async Task Handle_DocumentNotFound_ReturnsNull()
        {
            // Arrange
            var insertProvider = new TestOnAutoInsertProvider(">", canResolve: true);
            var endpoint = new OnAutoInsertEndpoint(Dispatcher, EmptyDocumentResolver, new[] { insertProvider }, TestLanguageServerWorkspaceFactory.Instance);
            var uri = new Uri("file://path/test.razor");
            var @params = new OnAutoInsertParams()
            {
                TextDocument = new TextDocumentIdentifier(uri),
