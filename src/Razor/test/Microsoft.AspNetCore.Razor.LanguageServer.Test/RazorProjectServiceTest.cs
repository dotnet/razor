...
#nullable enable
    ...
    {
        // Arrange
        var hostProject = new HostProject("C:/path/to/project.csproj", "C:/path/to/obj", RazorConfiguration.Default with { RootNamespace = "TestRootNamespace" });
    ...
    {
        // Arrange
        var hostProject = new HostProject("path/to/project.csproj", "path/to/obj", RazorConfiguration.Default with { RootNamespace = "TestRootNamespace" });
    ...
    public async Task IProjectInfoListener_UpdatedAsync_SameConfigurationDifferentRootNamespace_UpdatesRootNamespace()
    {
        ...
        Assert.Equal("ASP", notification.Older.Configuration.RootNamespace);
    ...
    public async Task CloseDocument_ClosesDocumentInAllOwnerProjects()
    {
        ...
        var ownerProjectKey1 = await _projectService.GetTestAccessor().AddProjectAsync(
            ProjectFilePath, IntermediateOutputPath1, RazorConfiguration.Default with { RootNamespace = RootNamespace }, displayName: null, DisposalToken);
        var ownerProjectKey2 = await _projectService.GetTestAccessor().AddProjectAsync(
            ProjectFilePath, IntermediateOutputPath2, RazorConfiguration.Default with { RootNamespace = RootNamespace }, displayName: null, DisposalToken);
        await _projectService.AddDocumentToPotentialProjectsAsync(DocumentFilePath, DisposalToken);
        await _projectService.OpenDocumentAsync(DocumentFilePath, s_emptyText, DisposalToken);
    ...
    public async Task IRazorProjectInfoListener_UpdatedAsync_AddsProjectWithSpecifiedConfiguration()
    {
        ...
        var configuration = new RazorConfiguration(RazorLanguageVersion.Version_1_0, "TestName", Extensions: [], RootNamespace);
