...
public partial class VSCodeService(IntegrationTestServices testServices)
{
    ...
    {
        if (!string.IsNullOrEmpty(testServices.Settings.VSCodePath) && File.Exists(testServices.Settings.VSCodePath))
        {
            _vsCodeExecutablePath = testServices.Settings.VSCodePath;
            testServices.Logger.Log($"Using configured VS Code path: {_vsCodeExecutablePath}");
            return;
        }

        var installDir = testServices.Settings.VSCodeInstallDir
            ?? throw new InvalidOperationException("VSCodeInstallDir not configured");

        Directory.CreateDirectory(installDir);

        _vsCodeExecutablePath = await _installer.EnsureVSCodeInstalledAsync(
            installDir,
            testServices.Settings.UseInsiders);
    }
