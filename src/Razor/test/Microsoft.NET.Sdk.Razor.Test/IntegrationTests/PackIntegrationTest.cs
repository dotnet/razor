    ...
    public class PackIntegrationTest : MSBuildIntegrationTestBase, IClassFixture<BuildServerTestFixture>
    {
        ...
        public async Task Pack_NoBuild_IncludeRazorContent_IncludesRazorViewContent()
        {
            ...
            Assert.NuspecContains(
                ...
                @"<files include=""any/netcoreapp3.1/Views/Shared/_Layout.cshtml"" buildAction=""Content"" />");

            Assert.NupkgContains(
                result,
                Path.Combine("bin", Configuration, "ClassLibrary.1.0.0.nupkg"),
                Path.Combine("contentFiles", "any", "netcoreapp3.1", "Views", "Shared", "_Layout.cshtml"));
        }
        ...
        public async Task Pack_NoBuild_Works_IncludesRazorAssembly()
        {
            ...
            if (!RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                ...
                Assert.NuspecContains(
                    result,
                    Path.Combine("obj", Configuration, "ClassLibrary.1.0.0.nuspec"),
                    $"<file src=\"{Path.Combine(Project.DirectoryPath, "bin", Configuration, "netcoreapp3.1", "ClassLibrary.Views.dll")}\" " +
                    $"target=\"{Path.Combine("lib", "netcoreapp3.1", "ClassLibrary.Views.dll")}\" />");

                Assert.NuspecDoesNotContain(
                    result,
                    Path.Combine("obj", Configuration, "ClassLibrary.1.0.0.nuspec"),
                    $"<file src=\"{Path.Combine(Project.DirectoryPath, "bin", Configuration, "netcoreapp3.1", "ClassLibrary.Views.pdb")}\" " +
                    $"target=\"{Path.Combine("lib", "netcoreapp3.1", "ClassLibrary.Views.pdb")}\" />");
            }
            ...
            Assert.NupkgContains(
                result,
                Path.Combine("bin", Configuration, "ClassLibrary.1.0.0.nupkg"),
                Path.Combine("lib", "netcoreapp3.1", "ClassLibrary.Views.dll"));
        }
        ...
        public async Task Pack_WithIncludeSymbols_IncludesRazorPdb()
        {
            ...
            Assert.NupkgContains(
                ...
                Path.Combine("lib", "netcoreapp3.1", "ClassLibrary.Views.dll"),
                Path.Combine("lib", "netcoreapp3.1", "ClassLibrary.Views.pdb"));
        }
