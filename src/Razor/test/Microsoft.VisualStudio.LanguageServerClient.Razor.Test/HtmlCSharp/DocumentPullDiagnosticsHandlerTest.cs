...
namespace Microsoft.VisualStudio.LanguageServerClient.Razor.HtmlCSharp
{
    public class DocumentPullDiagnosticsHandlerTest
    {
        ...
        public async Task HandleRequestAsync_ProjectionNotFound_ReturnsNull()
        {
            ...
            LSPDocumentSnapshot testDocument = new TestLSPDocumentSnapshot(testDocUri, 0, testVirtualDocument);
            ...
            var documentMappingProvider = Mock.Of<LSPDocumentMappingProvider>();
            var progressListener = Mock.Of<LSPProgressListener>();
            var referencesHandler = new FindAllReferencesHandler(requestInvoker, documentManager.Object, projectionProvider, documentMappingProvider, progressListener);
            referencesHandler.WaitForProgressNotificationTimeout = TestWaitForProgressNotificationTimeout;
            var referenceRequest = new ReferenceParams()
            {
                TextDocument = new TextDocumentIdentifier() { Uri = Uri },
        ...
        public async Task HandleRequestAsync_CSharpProjection_RemapsExternalRazorFiles()
        {
            ...
            var csharpVirtualDocument = new CSharpVirtualDocumentSnapshot(testCSharpDocUri, Mock.Of<ITextSnapshot>(), 2);
            var externalVirtualDocument = new CSharpVirtualDocumentSnapshot(externalUri, Mock.Of<ITextSnapshot>(), 5);
            var documentManager = CreateDocumentManager();
        ...
        private TrackingLSPDocumentManager CreateDocumentManager(params Uri[] uri)
        {
            var testVirtualDocUri = new Uri("C:/path/to/file2.razor.g");
            ...
            LSPDocumentSnapshot testDocument = new TestLSPDocumentSnapshot(Uri, 0, testVirtualDocument, csharpVirtualDocument);
