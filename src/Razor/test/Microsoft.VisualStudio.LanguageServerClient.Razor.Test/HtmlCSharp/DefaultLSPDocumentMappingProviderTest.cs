    ...
    public class DefaultLSPDocumentMappingProviderTest
    {
        ...
        private LSPRequestInvoker GetRequestInvoker(((RazorLanguageKind, Uri, TestTextEdit[]), (TestTextEdit[], int))[] mappingPairs)
        {
            ...
            foreach (var ((kind, uri, projectedEdits), (mappedEdits, version)) in mappingPairs)
            {
                ...
                requestInvoker
                    .Setup(r => r.CustomRequestServerAsync<RazorMapToDocumentEditsParams, RazorMapToDocumentEditsResponse>(LanguageServerConstants.RazorMapToDocumentEditsEndpoint, LanguageServerKind.Razor, requestParams, It.IsAny<CancellationToken>()))
                    .Returns(Task.FromResult(response));
            }
