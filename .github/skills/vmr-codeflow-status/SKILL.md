...
- **Backflow**: `dotnet/dotnet` → product repos (e.g., `dotnet/razor`)
...
./scripts/Get-CodeflowStatus.ps1 -PRNumber 52727 -Repository "dotnet/razor"

# Trace a specific fix through the pipeline
./scripts/Get-CodeflowStatus.ps1 -PRNumber 52727 -Repository "dotnet/razor" -TraceFix "dotnet/runtime#123974"

# Show individual VMR commits that are missing
./scripts/Get-CodeflowStatus.ps1 -PRNumber 52727 -Repository "dotnet/razor" -ShowCommits
...
./scripts/Get-CodeflowStatus.ps1 -Repository "dotnet/razor" -CheckMissing -Branch "main"
```
...
| `-Repository` | No | `dotnet/razor` | Target repo in `owner/repo` format |
...
darc get-subscriptions --target-repo dotnet/razor --source-repo dotnet/dotnet
