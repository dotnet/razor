---
name: formatting-log
description: Import Razor formatting log zips from Azure DevOps feedback tickets or GitHub issues into FormattingLogTest, validate whether the captured problem still reproduces, and if needed drive a minimal repro plus fix workflow.
---
...
1. Find the formatting log archive attached to the work item or issue.
...
1. Read the work item and comments with the Azure DevOps tools.
2. Look for attached files or direct download links whose names end in `Full_*_razor.zip` or `Range_*_razor.zip`.
3. If the attachment is not directly downloadable from the available tooling, ask the user for a local zip path instead of trying to work around auth.

### GitHub issue

1. Read the issue body and comments.
2. Look for uploaded zip attachments or linked archives with the same naming pattern.
3. Download the zip to a temporary folder.
...
.\.github\skills\formatting-log\scripts\Import-FormattingLog.ps1 `
  -ZipPath C:\Temp\Range_Index_razor.zip `
