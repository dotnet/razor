...
Analyze CI build status and test failures in Azure DevOps and Helix for dotnet repositories (razor, runtime, sdk, aspnetcore, roslyn, and more).
...
```powershell
# Analyze PR failures (most common) - defaults to dotnet/razor
...
| `-Repository` | Target repo (default: dotnet/razor) |
...
- **AzDO builds**: Comment `/azp run {pipeline-name}` on the PR
